...
import pandas as pd
import numpy as np
from keras.api.layers import Conv2D, MaxPooling2D, AveragePooling2D, ZeroPadding2D, Convolution2D
...
def create_model():
    ...
    x = Convolution2D(512, (1, 1), kernel_initializer="he_uniform", padding="same", name="DenseNet_initial_conv2D",
    ...
    x = Dropout(0.5)(x)
    x = Dense(16)(x)
    x = Lambda(lambda x: K.l2_normalize(x), output_shape=(1, 1, 16))(x)

    return x, img_input

def load_triplet_images(csvpath,target_size):
    data = pd.read_csv(csvpath,on_bad_lines='skip')
    trainX = []
    trainX1 = []
    trainX2 = []
    trainX3 = []
    for i in range(0,int(target_size/3)):
        mode = data.iloc[i, 4]
        #print(mode)
        img1 = cv2.imread("E:/vision/project/FECNet-master (1)/FECNet-master/data/train/"+data.iloc[i, 1])
        img2 = cv2.imread("E:/vision/project/FECNet-master (1)/FECNet-master/data/train/"+data.iloc[i, 2])
        img3 = cv2.imread("E:/vision/project/FECNet-master (1)/FECNet-master/data/train/"+data.iloc[i, 3])
        #print(img1)
...
if __name__=='__main__':
    train_x,train_y = load_triplet_images('E:/vision/project/FECNet-master (1)/FECNet-master/data/labels.csv',43200)
